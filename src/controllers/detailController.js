...
export const getImageDetail = async (req, res) => {
    try {
        ...
        if (data) { respsonseData(res, "Successfully handled", data, 200); }
        else { respsonseData(res, "No Image match your search", data, 404); }
    } catch {
...
export const getImageComments = async (req, res) => {
    try {
        let hinh_id = parseInt(req.params.hinh_id);
        let isHinhExisted = Boolean(await prisma.hinh_anh.findFirst({
            where: {
                hinh_id,
            },
        }))
        if (!isHinhExisted) {
            respsonseData(res, "No Image found, try with another image", "", 404);
        }
        else {
            ...
            if (data.length === 0) {
                respsonseData(res, "This image have no comment yet", "", 404);
...
export const getImageIsSaved = async (req, res) => {
    try {
        ...
        if (!isHinhExisted) {
            ...
            if (!data) { respsonseData(res, "Image have not saved yet", false, 404); }
            else { respsonseData(res, "Image saved", true, 200); }
        }
    } catch (error) {
        respsonseData(res, "Unexpected Error", "", 500);
    }
    ...
    try {
        ...
        if (!isHinhExisted) {
            ...
            if (req.file) {
                ...
                anh_dinh_kem = req.file.filename;
            }
